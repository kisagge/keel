 ...
 * 통째로 다시 쓰면 사람이 쓴 주석과 줄 순서가 사라진다. 그리고 재 보니(직접
 * `Y.Doc` 둘을 붙여서) Yjs 는 자리가 아니라 항목 단위로 지우므로, 같은 순간
 * 상대가 친 글자는 **소멸하지 않는다** — 대신 양쪽이 쓴 전문이 나란히 살아남아
 * 문서가 둘로 불어난다. 구간 수정일 때 4줄·진단 0 이던 것이 통째로 쓰기에서는
 * 7줄·진단 4 가 되고, 이음매에서 줄이 뭉개지고 같은 이름의 노드가 두 벌 생긴다.
 * 그래서 그 자리만 고친다.
 *
 * ## 먼저 전부 검사하고, 그다음에 얹는다
 *
