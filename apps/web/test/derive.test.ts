...
describe('장면 세우기', () => {
  ...
  /**
   * 끌기 중에는 그래프가 안 바뀌고 자리만 바뀐다.
   *
   * **"다시 파싱하지 않는다" 를 이 검사가 증명하지는 않는다** — 증명하는 것은
   * `sceneOf` 의 시그니처다. 소스가 아니라 `Graph` 를 받으므로 파싱할 글자가
   * 애초에 손에 없다. 여기서 묶는 것은 그 아래 사실이다: **같은 그래프로
   * 자리만 갈아 끼우면 장면이 그대로 따라온다.** 안 그러면 두 단으로 나눈
   * 값이 없어진다.
   */
  it('같은 그래프에 자리만 갈아 끼우면 장면이 따라온다', () => {
    const { graph } = parseSource('service a');
